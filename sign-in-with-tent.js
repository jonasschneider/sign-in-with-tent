(function() {
  ...
  if(!intentKlass || !startActivity) return;
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    for(var i=0;i<inputFields.length;i++) {
      ...
      if(!field.placeholder) field.placeholder = 'https://your.tent-entity.com';
      field.size = field.value ? field.value.length : field.placeholder.length;
      ...
      btn.addEventListener("click", function(e) {
        ...
        e.preventDefault();
        e.stopPropagation();
        startActivity.apply(window.navigator, [intent, function(userEntity) {
          if(typeof userEntity == 'string') {
            field.value = userEntity;
            field.form.submit();
          } else
            alert("Tent sign in failed.")
        }]);
        return false;
      });
    }
